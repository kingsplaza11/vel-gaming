...
from .models import GoldenRelicSession
from .services import dig
...
def start_game(request):
    ...
    session = GoldenRelicSession.objects.create(user=request.user, stake=stake)
    return Response({"session_id": session.id})

@api_view(["POST"])
@permission_classes([IsAuthenticated])
def dig_relic(request, session_id):
    session = GoldenRelicSession.objects.get(id=session_id, user=request.user)
    if not session.active:
        return Response({"error": "Game over"}, status=400)
    return Response(dig(session))
